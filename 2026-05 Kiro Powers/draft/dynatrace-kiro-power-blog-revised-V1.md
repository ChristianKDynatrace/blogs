# Dynatrace observability, one click away in Kiro
...
*[Figure 1 — Schematic flow of how Kiro activates the Dynatrace power and what it unlocks]*
...
Getting started takes only a few steps. Once installed, the Dynatrace power activates automatically when Kiro detects a relevant task — mention an incident, a slow service, or a production query, and the Dynatrace tools and steering load into context.
...
*Figure 2. Activating the Dynatrace power, confirming the connection, and querying live vulnerability data — all without leaving Kiro.*

## Conclusion

Kiro powers turn what used to be a stitching exercise — MCP servers here, steering files there, custom instructions somewhere else — into a single click. The Dynatrace power for Kiro brings observability into that model: live production insight, causal root cause analysis, and remediation grounded in real telemetry, all available the moment a developer needs them.
